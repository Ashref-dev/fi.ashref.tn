...
func Load(cmd *cobra.Command) (Config, error) {
	...
	if seconds := os.Getenv("AGCLI_TIMEOUT_SECONDS"); seconds != "" {
		v.Set("timeout", seconds+"s")
	}
	if openAIModel := os.Getenv("OPENAI_MODEL"); openAIModel != "" && os.Getenv("AGCLI_MODEL") == "" {
		v.Set("model", openAIModel)
	}
	if openAIBaseURL := os.Getenv("OPENAI_BASE_URL"); openAIBaseURL != "" && os.Getenv("AGCLI_OPENROUTER_BASE_URL") == "" {
...
func loadConfigFile(v *viper.Viper) error {
	...
	bases := []string{
		...
		filepath.Join(configDir, "ag-cli"),
	}
